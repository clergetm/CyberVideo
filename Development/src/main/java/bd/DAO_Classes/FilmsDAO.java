...
public class FilmsDAO<Film> extends DAO<Film>{
    ...
    public Film read(int id) {
        ...
        try{
            ...
            ResultSet result2 = this.connect.createStatement().executeQuery("SELECT CONCAT(actorFirstName,' ',actorLastName) AS actorName FROM FilmsActors NATURAL JOIN Actors WHERE filmID ="+id);
            ResultSet result3 = this.connect.createStatement().executeQuery("SELECT catName FROM FilmsCategories NATURAL JOIN Categories WHERE filmID ="+id);
            ...
            if(result.first()){
                ...
                while(result2.next()){
                    actors.add(result2.getString("actorName"));
                }
                ...
                while(result2.next()){
                    categories.add(result3.getString("actorName"));
                }
